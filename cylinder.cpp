...
Cylinder::Cylinder(bool isTube, bool hasBase, Point * centre, GLfloat width, GLfloat length, GLfloat xRotation, GLfloat yRotation, GLfloat  zRotation):
    centre(centre), length(length), width(width), tube(isTube), hasBase(hasBase),
...
void Cylinder::draw(GLUquadricObj * quadric, bool selected) {
    if(selected) glColor4f(1.0f,0.7,0.1,1.0);
    ...
    if(tube){
        ...
        gluCylinder(quadric,width/2.0f,width/2.0f,length,sliceInCircle,sliceInCircle);
    }
    else{
        ...
        gluCylinder(quadric,length/2.0f,length/2.0f,width,sliceInCircle,sliceInCircle);
    }
}
