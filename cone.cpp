...
Cone::Cone(Point *centre, GLfloat base, GLfloat height, GLfloat xRotation,
           GLfloat yRotation, GLfloat zRotation)
    : centre(centre), base(base), height(height),
      Shape(xRotation, yRotation, zRotation) {}

Cone::~Cone() { delete centre; }

void Cone::draw(GLUquadricObj *quadric, bool selected) {
  if (selected)
    glColor4f(0.8f, 0.0, 0.8, 1.0);
  else
    glColor4f(0.7, 0, 0.7, 1.0);
